const list = {
  ...
  inactiveButtonSave: "popup__save_disabled",
...
const showInputError = (object, formElement, inputElement, errorMessage) => {
  ...
  inputElement.classList.add(object.inputErrorActive);
...
const hideInputError = (object, formElement, inputElement) => {
  ...
  inputElement.classList.remove(object.inputErrorActive);
...
const disabledSubmitBtn = (object, buttonElement) => {
  buttonElement.classList.add(object.inactiveButtonSave);
  buttonElement.disabled = true;
};

const activeSubmitBtn = (object, buttonElement) => {
  buttonElement.classList.remove(object.inactiveButtonSave);
...
const toggleButtonState = (object, inputList, buttonElement) => {
  if (hasInvalidInput(inputList)) { //Если хотя бы один невалидный инпут, кнопка неактивна
    disabledSubmitBtn(object, buttonElement);
  } else {
    activeSubmitBtn(object, buttonElement);
  }
};
...
const disablesSubmitForm = (object, formElement) => {
  ...
  const buttonElement = formElement.querySelector(object.saveButtonSelector); // Ищем кнопку
...
const setEventListeners = (object, formElement) => {
  const inputList = Array.from(formElement.querySelectorAll(object.inputSelector) //Находим все поля внутри формы, делаем из них массив через Array.from
  );
  const buttonElement = formElement.querySelector(object.saveButtonSelector); // Находим в текущей форме кнопку отправки
